...
{
	song_t* answer = search(playlist -> list,song_id);

	if(!answer)
		return; 
	
	play_song(song_id);
	playlist -> last_song = answer;
}
	...
	if(empty(q))
		{
			...
			else if(playlist -> last_song == playlist -> list ->tail)
			{
				play_song(playlist -> list -> head -> data);
				playlist -> last_song = playlist -> list -> head;
			}
...
{
	if((is_empty(playlist -> list)) || (playlist->num_songs == 1) || k==0)
		return;

	else if(k == 1)
	{
		song_t *r1 = playlist -> list -> head;
		song_t *r2 = playlist -> list -> head -> next;

		for(int i = 1 ; i < (playlist -> num_songs) ; i++)
		{
			int value =  r1 -> data;
			r1 -> data = r2 -> data;
			r2 -> data = value;
	...
	{
		song_t *y1 = playlist -> list -> head;
		song_t *y2 = playlist -> list -> head;

		for(int iteration = 0; iteration < k ; iteration++)
			y2 = y2 -> next;
		
		for(int j = 0 ; (j+k) < (playlist -> num_songs) ; j++)
		{
			int value =  y1 -> data;
			y1 -> data = y2 -> data;
			y2 -> data = value;
...
{
	song_t *start_loop = playlist -> list -> head;
	song_t *check_loop = playlist -> list -> head;

	int switch_for_loops;

	while(start_loop && check_loop && check_loop ->next)
	{
		start_loop = start_loop -> next;
		check_loop = check_loop -> next -> next;

		if(start_loop == check_loop)
			{
			switch_for_loops = 1;
			break;
			}
